...
describe('createUserIntegration.test', () => {
	describe('createUser', () => {
		...
		const authenticatedClient = getAuthenticatedClientIntegration(process.env.OPENQ_API_SECRET, validSignatureFor0x1abc, true, true);

		const unauthenticatedClient_WRONG_API_KEY = getAuthenticatedClient('incorrect_secret', validSignatureFor0x1abc, null, null);
		const unauthenticatedClient_INVALID_EMAIL_OAUTH = getAuthenticatedClient(process.env.OPENQ_API_SECRET, invalidSignatureFor0x1abc, null, null);
